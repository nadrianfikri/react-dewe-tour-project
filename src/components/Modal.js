...
function ModalTitle(props) {
  ...
  return (
    <div className={`modal-title my-10 ${props.margin}`}>
      ...
      <button onClick={handleClose} className={`absolute top-12 right-10 text-2xl text-gray-400 close-modal z-50 transform rotate-45 ${props.top}`}>
        +
      </button>
...
function Modal(props) {
  return (
    <div id={props.id} className={`modal hidden overflow-auto h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 z-50 transition duration-500 ${props.margin}`}>
